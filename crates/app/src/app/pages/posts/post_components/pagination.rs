...
pub fn pagination(props: &PaginationProps) -> Html {
    ...
    html! {
        <div class="flex items-center justify-center space-x-4 mt-8">
            { if props.current_page != 1 { html! {
            <Link<Route>
                to={Route::PostIndex { page: props.current_page - 1 } }>
                <div
                    class="relative cursor-pointer duration-500 py-2 bg-white rounded-md shadow-sm px-2 bg-opacity-60 flex items-center w-24 justify-center text-gray-600 text-sm sm:text-base sm:px-4 sm:w-32 hover:shadow-md"
                >
                    <svg
                        class="w-5 h-5"
            ...
            >> } } else { html!{} }} { if props.has_next_page { html!{
            <Link<Route>
                to={Route::PostIndex { page: props.current_page + 1 } }>
                <div
                    class="relative cursor-pointer duration-500 py-2 bg-white rounded-md shadow-sm px-2 bg-opacity-60 flex items-center w-24 justify-center text-gray-600 text-sm sm:text-base sm:px-4 sm:w-32 hover:shadow-md"

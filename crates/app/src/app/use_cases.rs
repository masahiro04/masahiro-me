...
pub async fn fetch_posts_usecase(per_page: i32, offset: i32) -> post_repository::Result<Vec<Post>> {
    let app_state = AppState::from_config();
    app_state.fetch_posts(per_page, offset).await
}

pub async fn fetch_related_posts_usecase(category_ids: &str) -> post_repository::Result<Vec<Post>> {
    let app_state = AppState::from_config();
